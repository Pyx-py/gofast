...
func InitDefaultLogger(logZap bool) {
	loggerObj := New(log.New(os.Stdout, "\r\n", log.LstdFlags), config{
		SlowThreshold: 200 * time.Millisecond,
		LogLevel:      logger.Warn,
		Colorful:      true,
	})
	logIns := loggerObj.(*_logger)
	logIns.logZap = logZap
	Default = logIns
}
...
func (c *_logger) Printf(message string, data ...interface{}) {
	if c.logPath != "" && c.logZap {
		global.GF_LOG.Info(fmt.Sprintf(message, data...))
	} else {
